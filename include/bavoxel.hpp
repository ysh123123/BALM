...
namespace std {
template <>
struct hash<BALM1::VOXEL_LOC> {
  size_t operator()(const BALM1::VOXEL_LOC &s) const {
    using std::hash;
    using std::size_t;
...
template <typename T>
void down_sampling_voxel(pcl::PointCloud<T> &pl_feat, double voxel_size) {
  if (voxel_size < 0.01) {
    return;
  }

  unordered_map<BALM1::VOXEL_LOC, M_POINT> feat_map;
...
void down_sampling_voxel(BALM1::PL_VEC &pl_feat, double voxel_size) {
  unordered_map<BALM1::VOXEL_LOC, M_POINT> feat_map;
  uint plsize = pl_feat.size();
...
double feat_eigen_limit[2] = {8 * 8, 6 * 6};
...
class OCTO_TREE {
 public:
  ...
  bool calc_eigen() {
    ...
    feat_eigen_ratio = saes.eigenvalues()[1] / saes.eigenvalues()[ftype];
    ...
    return saes.eigenvalues()[2] / saes.eigenvalues()[1] < 10.0;
  }
  ...
  void recut(int layer, uint frame_head, pcl::PointCloud<PointType> &pl_feat_map) {
    if (octo_state == 0) {
      ...
      // 进行平面拟合
      is_plane = calc_eigen();  // calculate eigenvalue ratio
  ...
  void marginalize(int layer, int margi_size, vector<Eigen::Quaterniond> &q_poses, vector<Eigen::Vector3d> &t_poses,
    ...
    if (octo_state != 1 || layer == 0) {
      ...
      if (octo_state != 1) {
        ...
        // calc_eigen();
        const bool is_plane = calc_eigen();

        if (isnan(feat_eigen_ratio)) {
          feat_eigen_ratio = -1;
          return;
        }
        if (feat_eigen_ratio >= feat_eigen_limit[ftype] && is_plane) {
          pl_feat_map.push_back(ap_centor_direct);
        }
      }
    }
  ...
  void traversal_opt_calc_eigen() {
    ...
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> saes(covMat);
    feat_eigen_ratio_test = (saes.eigenvalues()[1] / saes.eigenvalues()[ftype]);
    is_plane = saes.eigenvalues()[2] / saes.eigenvalues()[1] < 10;
  }
  ...
  void traversal_opt(LM_SLWD_VOXEL &opt_lsv) {
    if (octo_state != 1) {
      ...
      if (feat_eigen_ratio_test > opt_feat_eigen_limit[ftype] && is_plane) {
...
class VOXEL_DISTANCE {
 public:
  ...
  void damping_iter() {
    ...
    Eigen::MatrixXd matA = Eigen::MatrixXd::Zero(6, 6);
    Eigen::VectorXd matB = Eigen::VectorXd::Zero(6);
    Eigen::VectorXd matX = Eigen::VectorXd::Zero(6);
