...
class VOXEL_MERGE {
 public:
  ...
  void reorganize(VOX_HESS &voxhess, pcl::PointCloud<PointType> &pl_send, pcl::PointCloud<PointType> &pl_cent,
                  vector<IMUST> &x_buf) {
    ...
    for (int i = 0; i < g1size; i++) {
      ...
      for (int j = 1; j < g2size; j++) {
        *sig_vec += *sig_vecs[group[j]];
        ...
        for (int k = 0; k < win_size; k++)
          if (plvec_tem[k].N != 0) (*plvec_voxel)[k] += plvec_tem[k];
      ...
      int process_size = 0;
      for (int j = 0; j < win_size; j++)
        if ((*plvec_voxel)[j].N != 0) process_size++;
      if (process_size < 2) {
        delete sig_vec;
        delete plvec_voxel;
        continue;
      ...
      double coe = 0;
      for (int j = 0; j < win_size; j++) coe += (*plvec_voxel)[j].N;
...
class OCTO_TREE_NODE {
 public:
  ...
  void cut_func(int ci) {
    ...
    // std::cout << "leave cut end" << std::endl;
    PLV(3)().swap(pvec_orig);
    PLV(3)().swap(pvec_tran);
    // std::cout << "swap pvec" << std::endl;
  }
  ...
  void recut(int win_count) {
    if (octo_state != 1) {
      ...
      for (int i = 0; i < win_count; i++) cut_func(i);
    ...
    // std::cout << "start leave recut" << std::endl;
    for (int i = 0; i < 8; i++)
      if (leaves[i] != nullptr) leaves[i]->recut(win_count);
    // std::cout << "eave recut end" << std::endl;
  }
  ...
  void to_margi(int mg_size, vector<IMUST> &x_poses, int win_count) {
    if (octo_state != 1) {
      ...
      for (int i = 0; i < 8; i++)
        if (leaves[i] != nullptr) leaves[i]->to_margi(mg_size, x_poses, win_count);
  }
  ...
  void tras_display(pcl::PointCloud<PointType> &pl_feat, int win_count) {
    if (octo_state != 1) {
      ...
      for (int i = 0; i < 8; i++)
        if (leaves[i] != nullptr) leaves[i]->tras_display(pl_feat, win_count);
    }
  }
  ...
  void tras_merge(VOXEL_MERGE &vlmg, int win_count) {
    if (octo_state != 1) {
      if (push_state == 1) {
        ...
        int psize = 0;
        for (int i = 0; i < win_count; i++) psize += vec_orig[i].size();

        if (psize > 100) vlmg.push_voxel(&sig_orig, &fix_point, center, direct, value_vector, plptr);
      }
    } else {
      for (int i = 0; i < 8; i++)
        if (leaves[i] != nullptr) leaves[i]->tras_merge(vlmg, win_count);
    }
  }
  ...
  void tras_opt(VOX_HESS &vox_opt, int win_count) {
    if (octo_state != 1) {
      ...
      if (push_state == 1) vox_opt.push_voxel(&sig_orig, &fix_point, decision, layer);
    } else {
      for (int i = 0; i < 8; i++)
        if (leaves[i] != nullptr) leaves[i]->tras_opt(vox_opt, win_count);
    }
  }
};
...
class BALM2 {
 public:
  ...
  double divide_thread_right(vector<IMUST> &x_stats, VOX_HESS &voxhess /*, vector<IMUST> &x_ab*/, Eigen::MatrixXd &Hess,
                             Eigen::VectorXd &JacT) {
    int thd_num = 4;
    double residual = 0;
    Hess.setZero();
    ...
    for (int i = 0; i < tthd_num; i++)
      mthreads[i] = new thread(&VOX_HESS::acc_evaluate2, &voxhess, x_stats, part * i, part * (i + 1), ref(hessians[i]),
  ...
  double divide_thread_left(vector<IMUST> &x_stats, VOX_HESS &voxhess, vector<IMUST> &x_ab, Eigen::MatrixXd &Hess,
                            Eigen::VectorXd &JacT) {
    int thd_num = 4;
    double residual = 0;
    Hess.setZero();
  ...
  void damping_iter(vector<IMUST> &x_stats, VOX_HESS &voxhess) {
    ...
    //     if (voxhess.plvec_voxels[i]->at(j).N != 0) planes[j]++;
    // }
    ...
    for (int i = 0; i < 10; i++) {
      ...
      // 限制整体更新步长，避免过大更新
      double update_norm = dxi.norm();
      double max_update = 0.5;  // 根据实际情况调整
      if (update_norm > max_update) dxi *= max_update / update_norm;
      ...
      for (int j = 0; j < win_size; j++) {
        ...
        // 限制每个旋转更新步长
        Eigen::Vector3d dtheta = dxi.block<3, 1>(DVEL * j, 0);
        double norm_dtheta = dtheta.norm();
        double theta_threshold = 0.1;  // 阈值示例
        if (norm_dtheta > theta_threshold) dtheta *= theta_threshold / norm_dtheta;
        Eigen::Matrix3d dR = Exp(dtheta);

        x_stats_temp[j].R = x_stats[j].R * dR;
        x_stats_temp[j].p = x_stats[j].p + dxi.block<3, 1>(DVEL * j + 3, 0);
      ...
      if (iter_stop(dxi, 1e-5)) break;

      if (fabs(residual1 - residual2) / residual1 < 1e-5) break;
    }
