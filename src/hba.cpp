...
void Layer::InitStorage(int total_layer_num_) {
...
void Layer::InitParameter(int pose_size_) {
  if (layer_num == 1)
  ...
  printf(
      ...
      layer_num, thread_num, pose_size, max_iter, part_length, gap_num, last_win_size, left_gap_num, tail, left_size,
      left_h_size, j_upper, downsample_size, voxel_size, eigen_ratio, reject_ratio);
}
HBA::HBA(int total_layer_num_, std::string data_path_, int thread_num_,vector<pose> pose_vec) {
  ...
  layers[0].pose_vec = pose_vec;
  // std::cout << "layers[0].pose_vec.size() " << layers[0].pose_vec.size() << std::endl;
  layers[0].InitParameter();
  layers[0].InitStorage(total_layer_num);
  ...
  for (int i = 1; i < total_layer_num; i++) {
    ...
    layers[i].InitParameter(pose_size_);
    layers[i].InitStorage(total_layer_num);
    // layers[i].data_path = layers[i - 1].data_path + "process1/";
  }
  // for (int i = 0; i < total_layer_num; i++) {
  //   cout << "i = "<<i<<" | path = "<<layers[i].data_path << endl;
  // }
  printf("HBA init done!\n");
}
HBA::HBA(int total_layer_num_, int thread_num_,vector<pose> pose_vec,std::vector<pcl::PointCloud<PointType>::Ptr> pc_vec) {
  total_layer_num = total_layer_num_;
  thread_num = thread_num_;
  // data_path = data_path_;
...
void HBA::UpdateNextLayerState(int cur_layer_num) {
...
std::vector<pose> HBA::PoseGraphOptimization() const {
  std::vector<pose> upper_pose, init_pose;
  upper_pose = layers[total_layer_num - 1].pose_vec;
  init_pose = layers[0].pose_vec;

  std::cout << "init_pose.size() " << init_pose.size() << std::endl;
      ...
      for (int j = 0; j < WIN_SIZE - 1; j++)
        for (int k = j + 1; k < WIN_SIZE; k++) {
          ...
          Vector6 << fabs(1.0 / init_cov[cnt - 1](0)), fabs(1.0 / init_cov[cnt - 1](1)),
              fabs(1.0 / init_cov[cnt - 1](2)), fabs(1.0 / init_cov[cnt - 1](3)), fabs(1.0 / init_cov[cnt - 1](4)),
  ...
  for (int i = 0; i < pose_size - 1; i++)
    for (int j = i + 1; j < pose_size; j++) {
      ...
      Vector6 << fabs(1.0 / upper_cov[cnt - 1](0)), fabs(1.0 / upper_cov[cnt - 1](1)),
          fabs(1.0 / upper_cov[cnt - 1](2)), fabs(1.0 / upper_cov[cnt - 1](3)), fabs(1.0 / upper_cov[cnt - 1](4)),
          fabs(1.0 / upper_cov[cnt - 1](5));
      gtsam::noiseModel::Diagonal::shared_ptr odometryNoise = gtsam::noiseModel::Diagonal::Variances(Vector6);
      gtsam::NonlinearFactor::shared_ptr factor(
          new gtsam::BetweenFactor<gtsam::Pose3>(i * pow(GAP, total_layer_num - 1), j * pow(GAP, total_layer_num - 1),
  ...
  // write_pose(init_pose, data_path);
  printf("pgo complete\n");
  return init_pose;
}