...
template <typename T>
void pub_pl_func(T &pl, std::string sensor_name) {
  mvt::protocol::PlotPointCloud PointCloud;
  PointCloud.mutable_header()->set_name(sensor_name);
  PointCloud.mutable_header()->set_time(PATC_GetTime);
  for (const auto &p : pl) {
    auto pp = PointCloud.mutable_pc()->add_point();
    pp->set_x(p.x);
    pp->set_y(p.y);
    pp->set_z(p.z);
    pp->set_intensity(p.intensity);
  }
  PATC_Pub(PointCloud);
}
...
int ReadPose(vector<double> &tims, PLM(3) & rots, PLV(3) & poss, string prename) {
  ...
  while (getline(inFile, lineStr)) {
    ...
    while (getline(ss, str, ',')) nums.push_back(stod(str));

    if (ord == 4) {
      for (int j = 0; j < 16; j++) aff(j) = nums[j];
...
void ReadFile(vector<IMUST> &x_buf, vector<pcl::PointCloud<PointType>::Ptr> &pl_fulls, string &prename) {
  prename = prename + "/datas/benchmark_realworld/";

  PLV(3) poss;
  PLM(3) rots;
  vector<double> tims;
  int pose_size = ReadPose(tims, rots, poss, prename);
  std::cout << "system is reading file..." << std::endl;
...
void DataShow(vector<IMUST> x_buf, vector<pcl::PointCloud<PointType>::Ptr> &pl_fulls) {
  IMUST es0 = x_buf[0];
...
int main(int argc, char **argv) {
  ...
  string file_path = "/mnt/d/ysh_projects/BALM";
  // n.param<string>("file_path", file_path, "");

  ReadFile(x_buf, pl_fulls, file_path);
  ...
  for (int iterCount = 0; iterCount < 1; iterCount++) {
    ...
    for (int i = 0; i < win_size; i++) cut_voxel(surf_map, *pl_fulls[i], x_buf[i], i);
